...
option = CodeGenerationOptions.generate_classes
...
def generate_comment(prompt):
    ...
    try:
        ...
        if "Please reduce the length of the messages" in str(e):
            ...
            completion = openai.ChatCompletion.create(model="gpt-3.5-turbo-16k", messages=[{"role": "user", "content": shorten_string(prompt)}])
        else:
        ...
        for cs_file_path, segment in segments:
            ...
            if segment in cs_content:
                ...
                if shorten_string(segment, 200) in cs_content:
                    ...
                    print("Found shortened string...")
                else:
                    print("Couldn't find segment in file content. Skipping...")
                    continue
            ...
            # Generate comment
            prompt = (
                f"Generate an xml comment (starts with /// and MUST include a <summary> tag) for the following {'class (ONLY for the class)' if option == CodeGenerationOptions.generate_classes else 'namespace (ONLY for the namespace)'} if it doesn't exist already. I want you to only respond with the provided comment, nothing else. The generated XML comment should always include a summary tag and the summary should be max 1-2 sentences. Code:\n{segment}."
                if option != CodeGenerationOptions.generate_fields_and_methods
                else f"Generate an xml comment (starts with /// and MUST include a <summary> tag) for the following code if it doesn't exist already. I want you to only respond with the provided comment, nothing else. The generated XML comment should always include a summary tag. Code:\n{segment}."
            )
            ...
            if not "///" in generated_comment or "summary" not in generated_comment:
            ...
            print("Generated segment (shortened to 500 chars):", shorten_string(generated_segment, 500))
