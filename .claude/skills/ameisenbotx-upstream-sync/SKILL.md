...
**On Linux there is no `USERPROFILE`** - use `$HOME/Downloads/AmeisenBotX` there. Every
`$USERPROFILE/Downloads` written in the snippets below means `$HOME/Downloads` on Linux; set `UP`
accordingly (`UP="$HOME/Downloads/AmeisenBotX"`) and the rest of the commands are unchanged.
