...
namespace AmeisenBotX.Windows
{
    public partial class MapWindow : Window
    {
        public MapWindow(AmeisenBot ameisenBot)
        {
            ...
            QuestPoiBrush = new SolidBrush(Color.Gold);
            QuestCompleteBrush = new SolidBrush(Color.FromArgb(255, 255, 215, 0)); // Gold
            QuestInProgressBrush = new SolidBrush(Color.FromArgb(255, 255, 255, 100)); // Lighter yellow
            QuestIconFont = new("Segoe UI", 12, System.Drawing.FontStyle.Bold);
            ...
            {
                AmeisenBot.Bot.Autopilot.QuestPulse.OnQuestsUpdated += () => { NeedToUpdateMap = true; };
            }
        ...
        ~MapWindow()
        {
            ...
            SubTextFont.Dispose();
            QuestPoiBrush.Dispose();
            QuestCompleteBrush.Dispose();
            QuestInProgressBrush.Dispose();
            QuestIconFont.Dispose();
            TextBrush.Dispose();
            TextFont.Dispose();
        }
        ...
        private Brush QuestPoiBrush { get; set; }
        private Brush QuestCompleteBrush { get; set; }
        private Brush QuestInProgressBrush { get; set; }
        private Font QuestIconFont { get; set; }
        ...
        private void CheckboxRenderQuestPois_Checked(object sender, RoutedEventArgs e)
        {
            AmeisenBot.Config.MapRenderQuestPois = true;
        }

        private void CheckboxRenderQuestPois_Unchecked(object sender, RoutedEventArgs e)
        {
            AmeisenBot.Config.MapRenderQuestPois = false;
        }
        ...
        private BitmapImage GenerateMapImage(Bitmap bitmap, Graphics graphics, int width, int height)
        {
            ...
            if (AmeisenBot.Bot.Player != null)
            {
                ...
                if (AmeisenBot.Config.MapRenderQuestPois)
                {
                    RenderQuestPois(halfWidth, halfHeight, graphics, Scale, playerPosition, playerRotation);
                }
        ...
        private void RenderQuestPois(int halfWidth, int halfHeight, Graphics graphics, float scale, Vector3 playerPosition, float playerRotation)
        {
            var quests = AmeisenBot.Bot.Autopilot?.QuestPulse?.ActiveQuests;
            ...
            foreach (var quest in quests.ToList())
            {
                ...
                {
                    if (obj.Location == Vector3.Zero) continue;
                    
                    Point pos = GetRelativePosition(playerPosition, obj.Location, playerRotation, halfWidth, halfHeight, scale);
                    
                    // Viewport culling
                    if (pos.X < -50 || pos.X > width + 50 || pos.Y < -50 || pos.Y > height + 50)
                        continue;
                    
                    string icon = quest.IsComplete ? "?" : "!";
                    Brush brush = quest.IsComplete ? QuestCompleteBrush : QuestInProgressBrush;
                    
                    RenderQuestIcon(pos.X, pos.Y, icon, quest.Title, brush, graphics);
                }
            }
        }

        private void RenderQuestIcon(int x, int y, string icon, string title, Brush iconBrush, Graphics graphics)
        {
            // Draw circular background
            int bgSize = 20;
            int bgOffset = bgSize / 2;
            graphics.FillEllipse(new SolidBrush(Color.FromArgb(180, 0, 0, 0)), x - bgOffset, y - bgOffset, bgSize, bgSize);
            graphics.DrawEllipse(new Pen(iconBrush, 2), x - bgOffset, y - bgOffset, bgSize, bgSize);
            
            // Draw the icon (? or !) centered
            SizeF iconSize = graphics.MeasureString(icon, QuestIconFont);
            graphics.DrawString(icon, QuestIconFont, iconBrush, x - (iconSize.Width / 2), y - (iconSize.Height / 2) - 1);
            
            // Draw quest title below
            if (!string.IsNullOrEmpty(title))
            {
                float titleWidth = graphics.MeasureString(title, TextFont).Width;
                graphics.DrawString(title, TextFont, TextBrush, x - (titleWidth / 2), y + bgOffset + 2);
            }
        }
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            checkboxRenderQuestPois.IsChecked = AmeisenBot.Config.MapRenderQuestPois;
